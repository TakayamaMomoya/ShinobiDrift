...
const float HANDLE_CURVE_MAG = -0.025f;  // 体勢からカーブへの倍率
const float SPEED_CURVE_MAG = 0.75f;  // 速度からカーブへの倍率(大きくなるほど曲がらなくなる)
...
void CPlayer::ManageSpeed(void)
{
	...
	{// 減速しているとき
		if (m_info.fSpeed > m_param.fSpeedMax)
			m_info.fSpeed += (m_info.fSpeedDest - m_info.fSpeed) * m_param.fFactAttenu * 3.0f;
		else
